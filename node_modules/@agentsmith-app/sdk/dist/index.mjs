...
var require_modifiers = __commonJS({
  "../node_modules/fast-redact/lib/modifiers.js"(exports, module) {
        ...
        if (wc) {
          ...
          for (var j = 0; j < wcKeys.length; j++) {
            ...
            if (consecutive) {
              ...
              level2 = i;
              ov = iterateNthLevel(wcov, level2 - 1, k, path3, afterPath, censor, isCensorFct, censorFctTakesPath, originalKey, n, nv, ov, kIsWc, wck, i, lastPathIndex, redactPathCurrent, store, o[originalKey], depth + 1);
            } else {
    ...
    function iterateNthLevel(wcov, level2, k, path3, afterPath, censor, isCensorFct, censorFctTakesPath, originalKey, n, nv, ov, kIsWc, wck, i, lastPathIndex, redactPathCurrent, store, parent, depth) {
      if (level2 === 0) {
        if (kIsWc || typeof wcov === "object" && wcov !== null && k in wcov) {
          if (kIsWc) {
            ov = wcov;
      ...
      for (const key in wcov) {
        if (typeof wcov[key] === "object") {
          ...
          iterateNthLevel(wcov[key], level2 - 1, k, path3, afterPath, censor, isCensorFct, censorFctTakesPath, originalKey, n, nv, ov, kIsWc, wck, i, lastPathIndex, redactPathCurrent, store, parent, depth + 1);
        }
      }
    }
    function tree() {
...
// ../src/utils/is-env.ts
var isProd = process.env.NEXT_PUBLIC_VERCEL_TARGET_ENV === "production" || process.env.VERCEL_TARGET_ENV === "production";
var isStage = process.env.NEXT_PUBLIC_VERCEL_TARGET_ENV === "staging" || process.env.VERCEL_TARGET_ENV === "staging";

// ../src/lib/logger.ts
var level = isProd ? "info" : "debug";
...
var routes = {
  ...
  studio: {
    ...
    organizationBilling: (organizationUuid) => `/studio/organization/${organizationUuid}/billing`,
    organizationBillingSuccess: (organizationUuid) => `/studio/organization/${organizationUuid}/billing/success`,
    project: (projectUuid) => `/studio/project/${projectUuid}`,
    editProject: (projectUuid) => `/studio/project/${projectUuid}/edit`,
    prompts: (projectUuid, openCreateModal) => `/studio/project/${projectUuid}/prompts${openCreateModal ? "?openCreateModal=true" : ""}`,
    editPrompt: (projectUuid, promptUuid) => `/studio/project/${projectUuid}/prompts/${promptUuid}/edit`,
    ...
    metrics: (projectUuid) => `/studio/project/${projectUuid}/metrics`,
  ...
  external: {
    ...
    stripe: {
      checkout: (organizationUuid, stripePriceId, seats) => `/stripe/checkout?organizationUuid=${organizationUuid}&stripePriceId=${stripePriceId}${seats ? `&seats=${seats}` : ""}`,
      portal: (organizationUuid) => `/stripe/portal?organizationUuid=${organizationUuid}`
    },
    discord: "/discord"
  },
  emails: {
...
var TemplateVariablesExtractor = class {
  constructor() {
    ...
    this.includeMap = /* @__PURE__ */ new Map();
    this.localVariables = /* @__PURE__ */ new Set();
  }
  /* ---------- Utilities ---------- */
  ensureVariablePath(path3, finalType = "STRING") {
    if (path3.length === 0) return null;
    let current = this.variableMap.get(path3[0]);
    if (!current) {
      current = createParsedVariable(path3[0], path3.length === 1 ? finalType : "JSON");
      this.variableMap.set(path3[0], current);
    }
  ...
  ensurePathOnVariable(root, path3, finalType = "STRING") {
    let current = root;
  ...
  traverseNode(node, ctx = {}) {
    ...
    switch (node.typename) {
      ...
      case "LookupVal": {
        const path3 = this.literalPathFromLookup(node);
        if (!path3) {
          this.traverseNode(node.val, ctx);
          this.traverseNode(node.target, ctx);
          break;
        }
        if (ctx.currentCollection && ctx.loopItemName && path3[0] === ctx.loopItemName && path3.length > 1) {
          this.ensurePathOnVariable(ctx.currentCollection, path3.slice(1));
        } else if (!(ctx.loopItemName && path3.length === 1 && path3[0] === ctx.loopItemName)) {
          if (!this.localVariables.has(path3[0])) {
            this.ensureVariablePath(path3);
          }
        }
        break;
      }
...
var compilePrompt = (promptContent, variables, promptLoader) => {
...
var VERSION_TYPE_LABELS = {
  [VERSION_TYPES.patch]: "Patch",
  [VERSION_TYPES.minor]: "Minor",
  [VERSION_TYPES.major]: "Major",
  [VERSION_TYPES.custom]: "Custom"
};
var VERSION_TYPE_DESCRIPTIONS = {
  [VERSION_TYPES.patch]: "A patch version indicates a bug fix or a small change that does not break existing functionality.",
  [VERSION_TYPES.minor]: "A minor version indicates a new feature or a significant change that does not break existing functionality.",
  [VERSION_TYPES.major]: "A major version indicates a significant change that may break existing functionality.",
  [VERSION_TYPES.custom]: "A custom version indicates a special case or label for a version, such as 1.2.3-rc.1 or 1.2.3-beta.1."
};
...
var OrganizationsService = class extends AgentsmithSupabaseService {
  ...
  async getOrganizationProjects(organizationUuid) {
    const { data, error } = await this.supabase.from("organizations").select("projects(id, uuid, name)").eq("uuid", organizationUuid).single();
    if (error) {
  ...
  async getOrganizationData(organizationUuid) {
    ...
    ).eq("uuid", organizationUuid).single();
    if (error) {
  ...
  async getOrganizationTierData(organizationUuid) {
    const { data, error } = await this.supabase.from("agentsmith_tiers").select("*, organizations!inner(uuid)").eq("organizations.uuid", organizationUuid).single();
    if (error) {
  ...
  async removeOrganizationUser(organizationUserId) {
    const { error } = await this.supabase.from("organization_users").delete().eq("id", organizationUserId);
    if (error) {
  ...
  async isUnderUserLimit(organizationId) {
  ...
  async isUnderProjectLimit(organizationId) {
...
var LLMLogsService = class extends AgentsmithSupabaseService {
  ...
  static createLogUuidSSE(logUuid) {
...
// ../src/utils/merge-included-variables.ts
var mergeIncludedVariables = (options) => {
  const { variables, includedPromptVariables } = options;
  const mergedVariables = /* @__PURE__ */ new Map();
...
// ../src/utils/stream-to-iterator.ts
  ...
  try {
    while (true) {
      ...
      try {
        ...
        yield { type: event.event || "message", data };
      } catch (e) {
...
// ../src/utils/accumulate-stream.ts
import merge2 from "lodash.merge";
var accumulateChatStreamToCompletion = async (stream) => {
  ...
  for await (const event of stream) {
    ...
    if (chunk.usage) {
      accumulatedCompletion.usage = merge2(accumulatedCompletion.usage, chunk.usage);
      ...
      accumulatedCompletion = merge2(accumulatedCompletion, chunk);
    }
  }
  const completion = {
...
var Prompt = class {
  ...
  async _initFromFileSystem() {
    const { meta, version, variables, content, includedPrompts } = await this.fetchPromptFromFileSystem(this.slug, this.argVersion);
    this.meta = meta;
    this.version = {
      ...version,
  ...
  async _initFromRemote() {
    ...
    if (fetchSpecificVersion) {
      ...
      ).eq("prompts.slug", this.slug).eq("version", this.argVersion).abortSignal(this.client.abortController.signal).single();
      if (error2) {
      ...
      const { prompts: prompts2, prompt_variables: prompt_variables2, prompt_includes: prompt_includes2, ...version2 } = data2;
      this.meta = prompts2;
      this.version = {
        ...version2,
      ...
      this.includedPrompts = prompt_includes2.map((ip) => ({
    ...
    const { data, error } = await this.client.supabase.from("prompt_versions").select(
      "uuid, version, config, content, prompt_variables(uuid, name, type, required, default_value), prompts!inner(uuid, name, slug), prompt_includes!prompt_version_id(prompt_versions!included_prompt_version_id(uuid, version, content, prompts(slug), prompt_variables(uuid, name, type, required, default_value)))"
    ).eq("prompts.slug", this.slug).eq("status", "PUBLISHED").abortSignal(this.client.abortController.signal);
    if (error) {
  ...
  async fetchPromptJsonFromFileSystem(slug) {
  ...
  async fetchPromptVersionJsonFromFileSystem(slug, version) {
  ...
  async fetchPromptVariablesJsonFromFileSystem(slug, version) {
  ...
  async fetchPromptContentFromFileSystem(slug, version) {
  ...
  async fetchPromptFromFileSystem(slug, initialVersion) {
  ...
  parseOverloadedArgs(args, isOptions) {
  ...
  async _compile(...args) {
    ...
    const isCompileOptions = (arg) => typeof arg === "object" && arg !== null && "globals" in arg;
    const [variables, options] = this.parseOverloadedArgs(args, isCompileOptions);
    ...
    const finalGlobalContext = merge3(globals, globalOverrides);
    const finalVariables = {
      ...variablesWithDefaults,
      global: finalGlobalContext
    };
    const promptLoader = (slug, version) => {
      const includedPromptVersion = version !== null && version !== "latest" ? this.includedPrompts.find((ip) => ip.slug === slug && ip.version === version) : this.includedPrompts.filter((ip) => ip.slug === slug).sort((a, b) => compareSemanticVersions(b.version, a.version))[0];
  ...
  async logCompletionToFile(options) {
    ...
    this.client.queue.add(async () => {
      ...
      const logDir = this.client.completionLogDirTransformer ? this.client.completionLogDirTransformer({
        logUuid,
        rawInput,
        rawOutput,
        prompt: this.meta,
  ...
  async _execute(...args) {
    ...
    const isExecuteOptions = (arg) => typeof arg === "object" && arg !== null && ("config" in arg || "globals" in arg);
    const [variables, options] = this.parseOverloadedArgs(args, isExecuteOptions);
    const { compiledPrompt, finalVariables } = await this.compile(variables, options);
    const { config: configOverrides } = options ?? {
      config: {}
    };
    const finalConfig = merge3(this.version.config, configOverrides, {
      usage: {
        include: true
      }
    });
    ...
    try {
      const { value: openrouterApiKey, error } = await organizationsService.getOrganizationKeySecret(
        organizationUuid,
      ...
      const streamEnabled = typeof configOverrides?.stream !== "undefined" ? configOverrides.stream : this.version.config.stream;
      if (streamEnabled) {
        ...
        const tokens = async function* () {
          for await (const chunk of streamToIterator(streamForTokens)) {
        ...
        const reasoningTokens = async function* () {
          for await (const chunk of streamToIterator(streamForReasoning)) {
        ...
        const toolCalls2 = async function* () {
          for await (const chunk of streamToIterator(streamForToolCalls)) {
        ...
        const completion2 = async () => {
          const stream = streamToIterator(streamForCompletion);
          return accumulateChatStreamToCompletion(stream);
        };
        this.client.queue.add(async () => {
          const fullCompletion = await accumulateChatStreamToCompletion(
            streamToIterator(streamForLogging)
          );
          const { data, error: error2 } = await logEntryPromise;
          const logEntry = data;
          const logUuid = logEntry?.log_uuid;
          if (error2) {
            this.client.logger.error(
              `Failed to create LLM log entry in Agentsmith, but continuing execution without logging. Error: ${error2.message}`
            );
          }
        ...
        return {
          ...
          stream: streamToIterator(streamForStream),
...
// src/AgentsmithClient.ts
var defaultAgentsmithDirectory = path2.join(process.cwd(), "agentsmith");
var AgentsmithClient = class {
  constructor(sdkApiKey, projectId, options = {}) {
    ...
    this.agentsmithApiRoot = options.agentsmithApiRoot ?? "https://agentsmith.dev";
  ...
  createDefaultLogger(logLevel) {
    const shouldLog = (level2) => {
      if (logLevel === "silent") return false;
      const levels = ["debug", "info", "warn", "error"];
      return levels.indexOf(level2) >= levels.indexOf(logLevel);
    };
    const noOp = () => {
    };
    return {
  ...
  async exchangeApiKeyForJwt(sdkApiKey) {
    try {
      ...
      const refreshJwtTimeout = (data.expiresAt - Date.now() / 1e3 - 60) * 1e3;
  ...
  async fetchGlobalsFromFileSystem() {
  ...
  async fetchGlobalsFromRemote() {
    try {
      const { data, error } = await this.supabase.from("global_contexts").select("*, projects(uuid, organizations(uuid))").eq("projects.uuid", this.projectUuid).abortSignal(this.abortController.signal).single();
      if (error) {
        this.fetchGlobalsPromise = null;
        throw error;
      }
